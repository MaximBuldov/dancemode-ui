...
export const ReportSummary = ({ reports }: IReportSummaryProps) => {
  ...
  reports.forEach(({ card, cash, profit, revenue, adv, beg, students, coupons, costs }) => {
    totalCash += cash;
    totalCard += card;
    totalRev += revenue;
    totalProf += profit;
    ...
    totalStu += students;
    totalCoup += coupons;
    totalCosts += costs?.reduce((acc, item) => acc + Number(item.sum), 0) || 0;
  });
  ...
  return (
    <>
      <Row key={1} style={{ backgroundColor: '#fafafa' }}>
        ...
        <Cell align="center" index={4}>{totalStu}</Cell>
        <Cell align="center" index={5}>${totalProf}</Cell>
